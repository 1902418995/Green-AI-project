...
def get_prune_threshold(model_list, percent):
    ...
    # Avoid clipping the highest threshold for all channels (the minimum of the maximum value of the gamma for each BN layer is the upper threshold limit)
    highest_thre = []
    for bnlayer in model_list.values():
        highest_thre.append(bnlayer.weight.data.abs().max().item())

    highest_thre = min(highest_thre)
    # Find the percentage of the subscript corresponding to highest_thre
